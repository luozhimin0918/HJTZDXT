...
public class WebSocketConnection implements WebSocket {
	...
	private void failConnection(int code, String reason) {
		...
		if (DEBUG)
			Log.d(TAG, "fail connection [code = " + code + ", reason = "
					+ reason);
	...
	public void connect(String wsUri, String[] wsSubprotocols,
		...
		try {
			...
			if (mWsUri.getScheme().equals("wss")) {
				throw new WebSocketException(
						"secure WebSockets not implemented");
			}
			...
			if (mWsUri.getHost() == null) {
				throw new WebSocketException(
						"no host specified in WebSockets URI");
			} else {
	...
	private void onClose(int code, String reason) {
		...
		if (mWsHandler != null) {
			try {
				if (reconnecting) {
					mWsHandler.onClose(ConnectionHandler.CLOSE_RECONNECT,
							reason);
				} else {
		...
		mMasterHandler = new Handler(Looper.getMainLooper()) {
			...
			public void handleMessage(Message msg) {
				...
				if (msg.obj instanceof WebSocketMessage.TextMessage) {
					...
					failConnection(
							WebSocketConnectionHandler.CLOSE_CONNECTION_LOST,
					...
					failConnection(
							WebSocketConnectionHandler.CLOSE_PROTOCOL_ERROR,
					...
					failConnection(
							WebSocketConnectionHandler.CLOSE_INTERNAL_ERROR,
							"WebSockets internal error ("
									+ error.mException.toString() + ")");
					...
					failConnection(
							WebSocketConnectionHandler.CLOSE_SERVER_ERROR,
							"Server error " + error.mStatusCode + " ("
									+ error.mStatusMessage + ")");
	...
	protected void createWriter() {
		...
		mWriterThread.start();
		mWriter = new WebSocketWriter(mWriterThread.getLooper(),
				mMasterHandler, mTransportChannel, mOptions);
	...
	protected void createReader() {
		...
		mReader = new WebSocketReader(mMasterHandler, mTransportChannel,
				mOptions, "WebSocketReader");
		mReader.start();
